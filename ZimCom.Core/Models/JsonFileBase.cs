...
public class JsonFileBase<T> : IJsonModel<T> {
    public override string ToString() => JsonSerializer.Serialize<JsonFileBase<T>>(this, options: new JsonSerializerOptions { WriteIndented = true });
    ...
    public static string GetFilePath() => Path.Combine(StaticLocalPathModule.GetLocalApplicationFolder(), $"{typeof(T).Name}.json");
}
